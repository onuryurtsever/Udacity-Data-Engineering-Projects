...
def process_song_file(cur, filepath):
    """
    - This function takes song data file from source and store into dimension tables which are songs and artists after process.
    """    
    # open song file
    ...
    song_data = song_data.fillna(0).values[0].tolist()
    cur.execute(song_table_insert, song_data)
    
    # insert artist record
    artist_data = song_df[['artist_id','artist_name','artist_location','artist_latitude','artist_longitude']]
    artist_data = artist_data.fillna(0).values[0].tolist()
    cur.execute(artist_table_insert, artist_data)


def process_log_file(cur, filepath):
    """
    - This function takes log data file from source and store into dimension tables which are users and time.
    - Lastly it stores into fact table which is songplays after process is completed.
    """  
    # open log file
...
def process_data(cur, conn, filepath, func):
    """
    - This function process each file for a given path.
    """  
...
def main():
    """
    - This is main function that use functions such as process_song_file, process_log_file, process_data.
